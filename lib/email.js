...
export async function sendOTPEmail(email, otp, type = 'register', opts = {}) {
  ...
  const userAvatar = opts.userAvatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(userName)}&background=e0e7ff&color=4f46e5&size=128&rounded=true` // Otomatis buat PP inisial jika kosong
  ...
  <head>
    ...
    <style>
      ...
      .otp-box { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 16px; padding: 24px; margin-bottom: 28px; }
      .otp-label { color: #64748b; font-size: 12px; text-transform: uppercase; letter-spacing: 1.5px; font-weight: 700; margin: 0 0 12px; }
      .otp-code { color: ${primaryColor}; font-size: 42px; font-weight: 800; letter-spacing: 12px; margin: 0; font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace; }
